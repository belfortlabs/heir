...
inline int64_t findBestBSGSRatio(llvm::ArrayRef<int32_t> diags, int64_t slots,
  ...
  for (int64_t n1 = 1; n1 < slots; n1 <<= 1) {
    ...
    // Float division with no nbN1 > 0 guard, exactly like Lattigo: for a
    // sparse diagonal set nbN1 reaches 0 with nbN2 > 0, and the resulting
    // +inf ratio must take the `> maxRatio` branch (0/0 -> NaN compares
    // false and continues). An integer mirror with a zero guard picks a
    // different N1 than the backend and generates the wrong galois keys.
...
/// whose BSGS baby-step size (giant-step stride) is exactly `n1`.
