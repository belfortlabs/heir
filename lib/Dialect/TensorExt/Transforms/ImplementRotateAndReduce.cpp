...
LogicalResult convertToLinearTransform(RotateAndReduceOp op) {
  ...
  if (static_cast<int64_t>(indices.size()) < numRows) {
    ...
    if (matchPattern(diagonals, m_Constant(&diagonalsAttr))) {
      ...
      for (int64_t index : indices) {
        rows.push_back(tensor::ExtractSliceOp::create(
            builder, op.getLoc(), rowType, diagonals,
            /*offsets=*/
            ArrayRef<OpFoldResult>{builder.getIndexAttr(index),
                                   builder.getIndexAttr(0)},
            /*sizes=*/
            ArrayRef<OpFoldResult>{
                one, builder.getIndexAttr(diagonalsType.getDimSize(1))},
            /*strides=*/ArrayRef<OpFoldResult>{one, one}));
      }
      diagonals = tensor::ConcatOp::create(builder, op.getLoc(), compactType,
                                           /*dim=*/0, rows);
    }
  }
