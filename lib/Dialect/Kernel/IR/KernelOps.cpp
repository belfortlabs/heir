...
LogicalResult LinearTransformOp::verify() {
  ...
  // The diagonals hold exactly the used diagonals, in the order the indices
  // name them: row k is the diagonal named by diagonal_indices[k]. Selection
  // from a fuller matrix is a gather at the producer, not a job for the
  // backend.
  int64_t numDiagonals = diagonalsType.getDimSize(0);
  int64_t numIndices = getDiagonalIndices().size();
  if (numDiagonals != numIndices) {
