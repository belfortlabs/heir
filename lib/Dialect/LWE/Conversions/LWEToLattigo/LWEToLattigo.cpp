...
struct ConvertKernelLinearTransformOp
  ...
  LogicalResult matchAndRewrite(
    ...
    auto linearTransformOp = lattigo::CKKSLinearTransformOp::create(
        ...
        adaptor.getDiagonals(), diagonalIndicesI32Attr, levelQAttr,
        logBSGSRatioAttr);
...
struct ConvertKernelPrepareLinearTransformOp
  ...
  LogicalResult matchAndRewrite(
    ...
    // Lattigo takes I32 diagonal indices.
    std::vector<int32_t> diagonalIndicesI32;
    for (auto val : op.getDiagonalIndices()) {
      diagonalIndicesI32.push_back(static_cast<int32_t>(val));
    }

    rewriter.replaceOpWithNewOp<lattigo::CKKSPrepareLinearTransformOp>(
        op, lattigo::CKKSLinearTransformationType::get(op.getContext()),
