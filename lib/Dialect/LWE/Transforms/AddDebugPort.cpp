...
void insertValidationOps(func::FuncOp op,
  ...
  auto insertValidate = [&](Value value, OpBuilder& b) {
    ...
    // ciphertexts before relinearization carries an s^2 ("Rx") term that the
    // decrypt-based debug hook cannot decrypt ("Rx should be removed before
    // decryption"). The relinearized result is debugged on the next op.
