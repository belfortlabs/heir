...
TEST_P(KernelImplementationTest, TestConv1dCwFcwStride2WithPadding) {
  ...
  // not (2*6=12 rounds to 16, 2*10=20 rounds to 32). The latter is the case
  // that goes wrong if the expanded matrix shape is re-derived from the padded
  // operand type with padding = 0, since the squat-diagonal collapse then runs
  // an extra rotate-and-add round.
