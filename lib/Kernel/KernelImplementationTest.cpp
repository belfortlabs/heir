...
// Absorbing the data packing into the filter's column space widens the
// diagonal layout to the full ciphertext. The kernel folds partial sums using
// its own column count, so that count must be the width the layout was built
// with, not the logical C*W of the expanded Toeplitz matrix.
//
// This shape makes the two differ: C*W = 12 rounds up to 16, while the
// absorbed layout is 32 wide. The fold count is 1 against 2, so a kernel that
// keeps using C*W sums only part of the contraction.
TEST(KernelImplementationTest, TestConv1dCwFcwAbsorbedPackingWidth) {
  ...
  auto runKernel = [&](const std::vector<std::vector<int>>& packedData,
    ...
    LiteralValue vectorInput = packedData[0];
    auto dag = implementHaleviShoup(vectorInput, matrixInput, matrixShape,
                                    DagType::intTensor(32, {numSlots}),
                                    /*zeroDiagonals=*/{}, /*unroll=*/true);
