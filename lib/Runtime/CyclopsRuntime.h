...
void addPreparedRequests(const T& value, EvaluationKeyRequest& request) {
  if constexpr (requires { value->AddRequiredRotations(request); }) {
    require(bool(value), "uninitialized prepared transform");
    value->AddRequiredRotations(request);
  } else if constexpr (requires { std::tuple_size<T>::value; }) {
    std::apply(
        [&](const auto&... fields) {
          (addPreparedRequests(fields, request), ...);
        },
        value);
  }
}
