// RUN: heir-opt --generate-param-ckks="first-mod-bits=55 scaling-mod-bits=40 min-slot-count=8" %s | FileCheck %s
...
// The last twelve primes must have the extra precision needed by CtS/EvalMod.
// Their size must also participate in security sizing: an all-40-bit chain
// would fit logN=15, while this chain needs logN=16.
...
// CHECK-SAME: logN = 16
// CHECK-SAME: Q = [{{[0-9]+}}, {{([0-9]{13})}}, {{([0-9]{13})}}, {{([0-9]{16}(, [0-9]{16}){11})}}]
