...
// CHECK: @conv2d_nchw
// CHECK-SAME: %[[arg0:.*]]: !secret.secret<tensor<1x1x10x10xf32>> {tensor_ext.kernel_info = #tensor_ext.kernel_info<resultShape = [1, 1, 10, 10], gapFactor = 1>, tensor_ext.layout = #[[layout1]]}
func.func @conv2d_nchw(%arg0: !secret.secret<tensor<1x1x10x10xf32>>) -> !secret.secret<tensor<1x4x5x5xf32>> {
  ...
  ^body(%input0: tensor<1x1x10x10xf32>):
    ...
    // CHECK-SAME: tensor_ext.kernel_info = #tensor_ext.kernel_info<resultShape = [1, 1, 10, 10], gapFactor = 2>
...
// CHECK-SAME: %[[arg0:.*]]: !secret.secret<tensor<1x1x10x10xf32>> {tensor_ext.kernel_info = #tensor_ext.kernel_info<resultShape = [1, 1, 10, 10], gapFactor = 1>
func.func @conv2d_stride_chain(%arg0: !secret.secret<tensor<1x1x10x10xf32>>) -> !secret.secret<tensor<1x1x3x3xf32>> {
  ...
  ^body(%input0: tensor<1x1x10x10xf32>):
    ...
    // CHECK: linalg.conv_2d_nchw_fchw
    // CHECK-SAME: tensor_ext.kernel_info = #tensor_ext.kernel_info<resultShape = [1, 1, 3, 3], gapFactor = 2>
