...
// CHECK: @split
// CHECK: %[[LT:.*]] = kernel.prepare_linear_transform %{{.*}} {diagonal_indices = array<i64: 0, 1>} : tensor<2x512xf64> -> <level = 0, slots = 512, log_bsgs_ratio = 0>
...
module attributes {backend.lattigo, scheme.ckks} {
  func.func @split(%ct: !ct) -> !ct {
    ...
    %0 = kernel.linear_transform %ct, %diagonals {diagonal_indices = array<i64: 0, 1>, kernel.test} : !ct, tensor<2x512xf64> -> !ct
    return %0 : !ct
  }
}
