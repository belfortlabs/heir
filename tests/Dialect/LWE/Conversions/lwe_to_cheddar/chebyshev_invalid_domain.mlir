// RUN: not heir-opt %s --lwe-to-cheddar 2>&1 | FileCheck %s

// A Chebyshev approximation on a non-canonical domain (here [0, 1]) cannot be
// lowered to the cheddar backend: CHEDDAR's EvalPoly only evaluates on [-1, 1],
// and we refuse rather than silently drop the domain (which would miscompute
// the activation). The higher-level pipeline must normalize the domain first.

// CHECK: cannot lower orion.chebyshev with approximation domain
// CHECK-SAME: only evaluates on [-1, 1]
...
module attributes {scheme.ckks, ckks.schemeParam = #ckks.scheme_param<logN = 16, Q = [36028797019488257, 1099512938497, 1099510054913, 1099507695617, 1099515691009, 1099516870657, 1099506515969, 1099504549889, 1099503894529, 1099503370241, 1099502714881], P = [2305843009211596801, 2305843009210023937, 2305843009208713217], logDefaultScale = 40>} {
  func.func @chebyshev(%ct: !ct_L10) -> !ct_L10 {
    %ct_0 = orion.chebyshev %ct {coefficients = [0.0, 0.75, 0.0, 0.25], domain_end = 1.000000e+00 : f64, domain_start = 0.000000e+00 : f64} : (!ct_L10) -> !ct_L10
    return %ct_0 : !ct_L10
  }
}
