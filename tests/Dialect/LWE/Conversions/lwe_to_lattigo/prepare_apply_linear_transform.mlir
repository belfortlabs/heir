...
module attributes {backend.lattigo, ckks.schemeParam = #ckks.scheme_param<logN = 13, Q = [36028797018652673, 35184372121601], P = [1152921504606994433], logDefaultScale = 45, encryptionTechnique = extended>, scheme.ckks} {
  ...
  // CHECK: %[[PREPARED:.*]] = lattigo.ckks.prepare_linear_transform %{{.*}}, %{{.*}}, %[[DIAGONALS]] {diagonal_indices = array<i32: 0, 1>, levelQ = 0 : i64, logBabyStepGiantStepRatio = 0 : i64, logSlots = 9 : i64} : ({{.*}}, tensor<2x2xf64>) -> !linear_transformation
  ...
  func.func @test_prepare_apply(%ct: !ct) -> !ct {
    ...
    %lt = kernel.prepare_linear_transform %diagonals {diagonal_indices = array<i64: 0, 1>} : tensor<2x2xf64> -> !prepared
