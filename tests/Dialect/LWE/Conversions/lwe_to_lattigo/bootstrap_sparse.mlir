...
// Two ckks.bootstrap ops with distinct sparse logSlots must lower to two
// separate bootstrap-evaluator func args, each tagged with its logSlots, and
// each lattigo.ckks.bootstrap must reference the matching evaluator.
...
// CHECK-SAME: %[[bEval1:[a-zA-Z0-9_]+]]: ![[bEvalType]] {lattigo.bootstrap_log_slots = 1 : i64}
// CHECK-SAME: %[[bEval2:[a-zA-Z0-9_]+]]: ![[bEvalType]] {lattigo.bootstrap_log_slots = 2 : i64}
// CHECK-SAME: %[[ct:[a-zA-Z0-9_]+]]: ![[ctType]]
// CHECK: %[[r0:.*]] = lattigo.ckks.bootstrap %[[bEval1]], %[[ct]]
// CHECK: lattigo.ckks.bootstrap %[[bEval2]], %[[r0]]
...
module attributes {ckks.schemeParam = #ckks.scheme_param<logN = 17, Q = [1106058412451299513, 1056763241666817029, 957769724367225479, 919081519653443687, 1030837924888066153, 1084354410096143723, 1135846243351935917, 1087115004561311021, 997960547764032911, 892538949448853293, 1002528331340998513, 1100798419621231379, 981696679688787961, 1061922508412786269], P = [1152921504606846976], logDefaultScale = 60>, scheme.ckks} {
  func.func @bootstrap(%ct: !ct_L2) -> !ct_L13 {
    %ct_0 = ckks.bootstrap %ct {logSlots = 1 : i64} : !ct_L2 -> !ct_L13
    %ct_1 = ckks.bootstrap %ct_0 {logSlots = 2 : i64} : !ct_L13 -> !ct_L13
    return %ct_1 : !ct_L13
  }
}
