...
Cyclops symbols. Only the server requires CUDA. Include HEIR's source root (or
its packaged runtime include root) and use Cyclops' CMake targets to obtain its
serialization dependencies and generated headers.
...
`Evaluate` optionally accepts a `DebugSink`. Its checkpoint contains borrowed
ciphertext pointers valid during the callback. The caller handles serialization,
transport, and client-side decryption; the GPU evaluator needs no secret key.
